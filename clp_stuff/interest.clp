...
(deftemplate investment-dev
	     (slot amount)
	     (slot period))

(deftemplate reference
  (slot max-pos))
...
(deffacts start-invest
	     ...
	     (investment-dev (amount 0)(period 0))
	     (max-pos 0)
...
(defrule calculate
	 (investment-data
	     ...
	     ?rif_per <- (investment-dev (amount ?am) (period
						       ?nper&:(< ?nper
								 ?nper_llim)))
	     ?rif_max <-(reference (max-pos ?nper))
	     ;;	     (not (investment-dev (period ?nper2&:(< ?nper ?nper2))))
	     =>
	     (bind ?delta (* (+ ?am ?q) (- ?inte 1) 0.74))
	     (bind ?nper1 (+ ?nper 1))
	     ;;	     (retract ?rif_per)
	     (modify ?rif_max (max-pos ?nper1))
	     (assert (investment-dev (amount (+ ?am ?delta ?q)) (period
							  ?nper1))))
;; 	     (assert
;; 		(investment-data
;; 			(amount (+ ?am ?delta ?q))
