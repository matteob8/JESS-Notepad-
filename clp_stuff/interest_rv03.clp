...
(deftemplate investment-data-full
	     (slot amount)
	     (slot quota)
	     (slot interest)
	     (slot periods))
...
(deffacts start-invest
  	(investment-data 0 1000 1.02 15))


(defrule calculate
	 (investment-data ?am ?q ?inte ?nper&:(< 0 ?nper))
	     =>
	     (bind ?delta (* (+ ?am ?q) (- ?inte 1) 0.74))
	     (bind ?nper1 (- ?nper 1))
	     (assert
			(investment-data (+ ?am ?delta ?q) ?q ?inte ?nper1)))
